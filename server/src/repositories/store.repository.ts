...
export async function listStores() { return prisma.store.findMany({ orderBy: { id: 'asc' } }) }
export async function getStore(id: number) { return prisma.store.findUnique({ where: { id }, include: { products: true } }) }
export async function createStore(data: { name: string; location?: string | null }) { return prisma.store.create({ data }) }
export async function updateStore(id: number, data: { name?: string; location?: string | null }) { return prisma.store.update({ where: { id }, data }) }
export async function deleteStore(id: number) { return prisma.store.delete({ where: { id } }) }
...
export async function storeSummary(id?: number, lowStockThreshold = 5) {
  ...
  for (const p of products) {
    ...
    if (p.quantity <= lowStockThreshold) summary.lowStock += 1
