...
export async function list(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const filters = {
      storeId: req.query.storeId ? Number.parseInt(req.query.storeId as string, 10) : undefined,
      ...
      minQty: req.query.minQty ? Number.parseInt(req.query.minQty as string, 10) : undefined,
      maxQty: req.query.maxQty ? Number.parseInt(req.query.maxQty as string, 10) : undefined,
...
export async function create(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (!parsed.success)
      return res.status(400).json({ error: { message: 'Validation failed', details: parsed.error.format() } })
...
export async function update(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (parsed.data.storeId)
      data.store = { connect: { id: parsed.data.storeId } }
